...
class TarDataset(Dataset):
    def __init__(self, tar_u2i, item_num):  
        super(TarDataset, self).__init__()
        self.tar_u2i = tar_u2i
        self.tar_u2i_dok = self.tar_u2i.todok()
        self.item_num = item_num

        self.all_users = tar_u2i.nonzero()[0]
    ...
    def __getitem__(self, idx):
        ...
        pos = self.tar_u2i.nonzero()[1][idx]
        neg = np.random.randint(0, self.item_num)
        while (user, neg) in self.tar_u2i_dok:
            neg = np.random.randint(0, self.item_num)
        return user, pos, neg
